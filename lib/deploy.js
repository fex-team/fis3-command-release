...
function doPackTasks(tasks, done) {
  var asyncTasks = [];
...
exports = module.exports = function(opt, collection, total) {
   ...
   * @return {[type]}
   */
  function createDeployTasks(depolyConf, files) {
    var tasks = [];
  ...
  settings = fis.env().get('deploy', []);

  //downward compatibility
  // fis.util.map(settings, function(name, conf) {
  //   bindConfInfo(conf, DEFAULT_DEPLOY_KEY, name, false);
  // });
  bindConfInfo(settings, settings.type || DEFAULT_DEPLOY_KEY, 'default', false);

  //add default deploy module
  if (!fis.env().get('modules.deploy')) {
    fis.env().set('modules.deploy', 'default');
  }
  ...
  fis.util.pipe('deploy', function(processor, pluginSettings, key) {
    ...
    bindConfInfo(pluginSettings, key, 'default', !!processor.fullpack);
    Array.isArray(settings) ? settings.push(pluginSettings) : fis.util.assign(settings, pluginSettings);
  });

  if (Array.isArray(settings) && !settings.length) {
    settings = bindConfInfo({}, DEFAULT_DEPLOY_KEY, 'default', false);
  }
  ...
  if (~opt.dest.indexOf(',')) {
    fis.log.error('invalid deploy path');
  }

  (function() {
    var dest = settings;
    var destName = opt.dest;
    var target = dest._fullpack ? packDeployConfs : deployConfs;
    ...
    dest.forEach(function(item) {
      ...
      if (!item.to && item._type == DEFAULT_DEPLOY_KEY) {
        if (
  ...
  var tasks = createDeployTasks(deployConfs, collection);
