var checkIgnore = require('./checkignore.js');
var _ = fis.util;
var chokidar = require('chokidar');

var patterns, root;

// 判断新加的文件是否满足用户指定。
function match(path) {
  patterns = patterns || fis.media().get('project.files', []).map(function(pattern) {
    var negate = false;

    if (pattern[0] === '!') {
      negate = true;
      pattern = pattern.substring(1);
    }

    return {
      negate: negate,
      reg: _.glob(pattern)
    };
  });

  path.indexOf(root) === 0 && (path = path.substring(root.length));

  var hitted = false;
  patterns.forEach(function(item) {
    if (hitted) {
      if (item.negate && item.reg.test(path)) {
        hitted = false;
      }
    } else {
      hitted = item.negate  !== item.reg.test(path);
    }
  });

  return hitted;
}

var child_process = require('child_process');

// 当监听到 fis-conf.js 文件变化的时候，kill self 重启一个新进程。
function respawn() {
  // destory self
  watcher.unwatch().close();
  fis.removeAllListeners();

  var argv = process.argv;
  fis.log.info('Detect `fis-conf.js` modified, respawn `fis release %s`.', argv.slice(3).join(' '));

  var child = child_process.spawn(argv[0], argv.slice(1), { stdio: 'inherit' });
  child.on('exit', function (code, signal) {
    process.on('exit', function () {
      if (signal) {
        process.kill(process.pid, signal);
      } else {
        process.exit(code);
      }
    });
  });
  return child;
}

var watcher;
function watch(options, next) {
  root = fis.project.getProjectPath();

  var opts = {
    usePolling: false,
    persistent: true,
    ignoreInitial: true,
    followSymlinks: false,
    ignored: function(path) {

      // normalize path
      path = path.replace(/\\/g, '/');
      path.indexOf(root) === 0 && (path = path.substring(root.length));

      // todo 在那时不支持 -f 参数指定其他配置文件。
      if (path === '/fis-conf.js') {
        return false;
      }

      return checkIgnore(path);
    }
  };

  _.assign(opts, fis.get('project.watch', {}));

  var lastTime = 0;
  var busy = false;
  function done() {
    busy = false;
  }

  var files = fis.project.getSource();
  options.srcCache = options.srcCache || [];
  fis.util.map(files, function (subpath, file) {
    options.srcCache.push(file.realpath);
  });

  var safePathReg = /[\\\/][_\-.\s\w]+$/i;
  function listener(type) {
    return function(path) {

      if (path && safePathReg.test(path)) {
        var modified = false;

        path = fis.util(path);

        if (path === root + '/fis-conf.js') {
          return respawn();
        }

        if (type === 'change') {
          if (~options.srcCache.indexOf(path)) {
            modified = true;
          } else {
            // not in list, so treat it as `add`.
            type = 'add';
          }
        }

        if (type === 'add') {
          ~options.srcCache.indexOf(path) || match(path) &&
            (options.srcCache.push(path), modified = true);
        } else if (type === 'unlink') {
          var idx = options.srcCache.indexOf(path);

          if (~idx) {
            options.srcCache.splice(idx, 1);
            modified = true;
          }
        } else if (type === 'unlinkDir') {
          var toDelete = [];

          options.srcCache.forEach(function(realpath, index) {
            if (realpath.indexOf(path) === 0) {
              toDelete.unshift(index);
            }
          });

          toDelete.forEach(function(index) {
            opt.srcCache.splice(index, 1);
            modified = true;
          });
        }

        // Nothing happend!
        if (!modified) {
          return;
        }
      }

      // 没有 release 完，或者离上次 release 时间小于 200ms.
      // watch 可能同时触发好几种事件。
      if (busy || (Date.now() - lastTime) < 200)return;
      busy = true;
      lastTime = new Date();
      next(null, options, done);
    }
  }

  watcher = chokidar
    .watch(root, opts)
    .on('add', listener('add'))
    .on('change', listener('change'))
    .on('unlink', listener('unlink'))
    .on('unlinkDir', listener('unlinkDir'))
    .on('error', function(err) {
      err.message += fis.cli.colors.red('\n\tYou can set `fis.config.set("project.watch.usePolling", true)` fix it.');
      fis.log.error(err);
    });

  opts.ignoreInitial && listener('inital')();
}

module.exports = watch;
