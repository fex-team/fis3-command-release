  ...
  function done(error, ret) {
    ...
    if (!error) {
      ...
      if (affectedList.length) {
        getAffectedFiles(latestTotal, affectedToCheck, false).forEach(function(filepath) {
          ~affectedList.indexOf(filepath) || affectedList.unshift(filepath);
        });

        // console.log(affectedToCheck, affectedList);
  ...
  function listener(type) {
    return function(path) {
      ...
      if (path && safePathReg.test(path)) {
        ...
        getAffectedFiles(latestTotal, affectedToCheck, !affectedList.length).forEach(function(filepath) {
          ~affectedList.indexOf(filepath) || affectedList.unshift(filepath);
        });
      }
